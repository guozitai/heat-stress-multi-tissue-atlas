...
| `annotate_celltypes.R` | Cell-type annotation of the integrated PBMC object, end to end: SingleR at cluster level against `celldex::MonacoImmuneData` with both the main and the fine label sets, and the marker-based adjudication of the clusters whose two predictions disagree. This is the workflow behind the `celltype` labels in the deposited object. |
| `evidence_table.R` | Builds `cluster_annotation_evidence.csv` from the annotated object |
| `cluster_annotation_evidence.csv` | Per-cluster annotation evidence for all 25 clusters: cell count, SingleR main and fine predictions, final label, whether the two agreed, and the detection rate of 14 canonical lineage markers |
...
- Key packages: Seurat 5.1.0, SingleR 2.8.0, celldex 1.16.0, hdWGCNA 0.4.11, CellChat 2.2.0.9001, clusterProfiler 4.14.3, DESeq2 1.46.0
